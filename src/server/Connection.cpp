...
// // 新增方法
// bool Connection::needs100Continue() const
// {
//     return request_() && request()->needs100Continue() && !continue_sent_;
// }
...
//     ssize_t sent = ::send(getFd(), response.c_str(), response.length(), MSG_NOSIGNAL);

//     if (sent > 0)
//     {
//         std::cout << "[FD=" << getFd() << "] Sent 100 Continue" << std::endl;
//         continue_sent_ = true;
//         request()->set100ContinueSent();
//     }
//     else
//     {
//         std::cout << "[FD=" << getFd() << "] Failed to send 100 Continue: " << strerror(errno) << std::endl;
//     }
// }
...
void Connection::resetForKeepAlive()
{
    ...
    std::cout << "[FD=" << getFd() << "] Reset for keep-alive" << std::endl;
}
